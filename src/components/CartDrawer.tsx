    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <Button variant="outline" size="icon" className="relative">
          ...
          {totalItems > 0 && (
            <Badge className="absolute -top-2 -right-2 h-5 w-5 rounded-full p-0 flex items-center justify-center text-xs bg-scribe text-white">
              {totalItems}
            </Badge>
          )}
        </Button>
          ...
          {items.length === 0 ? (
            ...
            <>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t bg-background">
                ...
                <Button onClick={handleCheckout} className="w-full bg-scribe hover:bg-scribe/90" size="lg" disabled={items.length === 0 || isLoading || isSyncing}>
